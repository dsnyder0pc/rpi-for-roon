...
if [[ "$MODEL" == *"Raspberry Pi 5"* ]]; then
    for BUS in usb1 usb3; do
        ...
        if [ -n "$MATCH" ]; then
            ...
            if [ "$HITS" -gt 100 ]; then
                ...
                # Use MATCH directly for the device label (e.g., xhci-hcd:usb3)
    ...
    MATCH=$(grep "xhci_hcd" /proc/interrupts)
    if [ -n "$MATCH" ]; then
        IRQ=$(echo "$MATCH" | awk -F: '{print $1}' | tr -d ' ')
        DEV="xhci_hcd"
        TARGET_IRQS="$IRQ"
        TARGET_DEVS="$DEV"
        echo "Detected Active DAC path on xhci_hcd (IRQ $IRQ)"
    fi
fi

# 3. Final Validation & Runtime Application
...
if [ -f "$ISO_CONFIG" ] && grep -q "ISOLATED1=\"$ISO_CORES\"" "$ISO_CONFIG"; then
    ...
    # Improved Regex: Matches "ID ", " ID ", or " ID" at start/middle/end
    for IRQ in $TARGET_IRQS; do
    ...
    # Cleanup: Remove double spaces and fix empty leading space in quotes
