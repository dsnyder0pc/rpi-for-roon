...
**Validación actual:**
Estas instrucciones fueron probadas por última vez con **AudioLinux V5** (Imagen: `audiolinux_pi4-pi5_530`, Versión del menú: `544`).
...
2.  [Preparación inicial de la imagen](#2-preparación-inicial-de-la-imagen)
3.  [Configuración del sistema base (Realizar en ambos dispositivos)](#3-configuración-del-sistema-base-realizar-en-ambos-dispositivos)
...
5.  [Configuración de red de punto a punto](#5-configuración-de-red-de-punto-a-punto)
6.  [Acceso SSH cómodo y seguro](#6-acceso-ssh-cómodo-y-seguro)
7.  [Optimizaciones comunes del sistema](#7-optimizaciones-comunes-del-sistema)
8.  [Instalación y configuración del software Diretta](#8-instalación-y-configuración-del-software-diretta)
9.  [Opción de integración de Roon](#9-opción-de-integración-de-roon)
10. [Opción de integración de UPnP](#10-opción-de-integración-de-upnp)
11. [Opción de integración de NAA](#11-opción-de-integración-de-naa)
12. [Opción de integración de LMS](#12-opción-de-integración-de-lms)
13. [Opción de integración de AES](#13-opción-de-integración-de-aes)
14. [Apéndice 1: Control de ventilador Argon ONE opcional](#14-apéndice-1-control-de-ventilador-argon-one-opcional)
15. [Apéndice 2: Control remoto IR opcional](#15-apéndice-2-control-remoto-ir-opcional)
16. [Apéndice 3: Modo purista opcional](#16-apéndice-3-modo-purista-opcional)
17. [Apéndice 4: Interfaz web de control del sistema opcional](#17-apéndice-4-interfaz-web-de-control-del-sistema-opcional)
18. [Apéndice 5: Comprobaciones de estado del sistema](#18-apéndice-5-comprobaciones-de-estado-del-sistema)
19. [Apéndice 6: Ajuste de rendimiento en tiempo real opcional](#19-apéndice-6-ajuste-de-rendimiento-en-tiempo-real-opcional)
20. [Apéndice 7: Optimizaciones de IRQ e hilos opcionales](#20-apéndice-7-optimizaciones-de-irq-e-hilos-opcionales)
21. [Apéndice 8: Velocidades de red puristas opcionales](#21-apéndice-8-velocidades-de-red-puristas-opcionales)
22. [Apéndice 9: Optimización de tramas jumbo opcional](#22-apéndice-9-optimización-de-tramas-jumbo-opcional)
23. [Apéndice 10: Actualizaciones del sistema opcionales](#23-apéndice-10-actualizaciones-del-sistema-opcionales)
...
1.  Inserte la tarjeta microSD en la **primera** Raspberry Pi, conéctela a su red y enciéndala. **Nota:** Si está utilizando la carcasa Argon ONE, es posible que escuche ruido del ventilador. No se preocupe. Una vez que haya terminado con la configuración de Diretta, hay instrucciones en el [Apéndice 1](#14-appendix-1-optional-argon-one-fan-control) para solucionar el ruido del ventilador.
...
3.  En la siguiente pantalla, seleccione **UPDATE system**, siga las indicaciones y deje que el proceso se complete.
...
5.  Seleccione **SELECT/Update kernel**. Si la versión que aparece junto a "Your kernel" es diferente de la versión indicada después de "Audiolinux LTS RT LTO", seleccione la opción 1, **Audiolinux LTS RT LTO (default)**, para actualizar el kernel.
6.  Salga del sistema de menús para volver a la terminal.
...
3.  **Configurar la traducción de direcciones de red (NAT):**
    ...
    # Vaciar todas las reglas antiguas de la memoria
    flush ruleset

    # Crear una tabla de tipo 'ip' (IPv4) llamada 'my_table'
    table ip my_table {

        # === Regla 2: Redirección de puertos (DNAT) ===
        # Esta cadena se engancha a la ruta 'prerouting' para el NAT
        chain prerouting {
            type nat hook prerouting priority dstnat;

            # Redirigir el puerto 5101 del Host al puerto 172.20.0.2:5001 del Target
            tcp dport 5101 dnat to 172.20.0.2:5001
        }

        # === Regla 3: Permitir el tráfico reenviado (FILTER) ===
        # Esta cadena se engancha a la ruta 'forward' para el filtrado de paquetes
        chain forward {
            type filter hook forward priority 0;

            # De forma predeterminada, descartar (drop) todo el tráfico reenviado
            policy drop;

            # Permitir las conexiones ya establecidas o relacionadas
            ct state established,related accept

            # Permitir el tráfico NUEVO que coincida con su regla de redirección de puertos
            ip daddr 172.20.0.2 tcp dport 5001 ct state new accept

            # Permitir todo el demás tráfico NUEVO procedente de la subred del Target
            ip saddr 172.20.0.0/24 accept
        }

        # === Regla 1: Acceso a Internet (MASQUERADE) ===
        # Esta cadena se engancha a la ruta 'postrouting' para el NAT
        chain postrouting {
            type nat hook postrouting priority 100;

            # Aplicar NAT (Masquerade) al tráfico de su subred que sale
            # por cualquier interfaz que empiece por 'enp', 'enu' o 'wlp'
...
if ! grep -q "AddKeysToAgent yes" "$SSH_CONFIG_FILE"; then
  ...
  # Usar un archivo temporal para anteponer los ajustes
...
if grep -q "Host diretta-host" "$SSH_CONFIG_FILE"; then
  ...
  # Añadir la nueva configuración usando un heredoc para mayor claridad
...
### 7.7. Reiniciar
Reinicie (primero el Target, luego el Host) para actualizar el estado de los servicios shadow:
```bash
sudo sync && sudo reboot
```
...
>
> Su sistema base Diretta y Roon ahora debería ser completamente funcional. Para verificar todos los servicios y conexiones, proceda al [**Apéndice 5**](#18-appendix-5-system-health-checks) y ejecute el comando universal **System Health Check** (Comprobación de estado del sistema) tanto en el Host como en el Target.
>
> ---

---

## 9. Opción de integración de Roon

**Objective:** Si es suscriptor de Roon, esta sección es para usted. Siga estos pasos para instalar Roon Bridge, habilitar su nueva Zona Diretta y reproducir música desde Roon.

1.  Ejecute `menu` en el Host si regresó a la terminal después del paso anterior, de lo contrario vaya al **Main menu**.
...
> ---
> ### ✅ Punto de control: Verificar la integración con Roon
>
> Seleccione su nueva zona Diretta en la aplicación de control de Roon y reproduzca algo de música.
>
> Ahora es un buen momento para regresar al [**Apéndice 5**](#18-appendix-5-system-health-checks) y ejecutar el comando universal **System Health Check** (Comprobación de estado del sistema) tanto en el Host como en el Target para verificar la configuración.
>
> ---

---

## 10. Opción de integración de UPnP

**Objetivo:** Habilitar las capacidades de renderizado de medios UPnP/DLNA en el Diretta Host utilizando MPD (Music Player Daemon) y UPMPDCLI, permitiendo la compatibilidad con puntos de control y reproductores ascendentes como Audirvāna, JRiver Media Center, mconnect o BubbleUPnP.
...
if grep -q 'Custom Diretta ALSA Audio Output' /etc/mpd.conf; then
  echo "Salida de audio ALSA de Diretta ya configurada"
else
...
```bash
# 1. Descubrir dinámicamente la interfaz de enlace ascendente LAN activa (garantizando un único nombre de interfaz)
UPNP_IFACE=$(ip route show default | awk '{print $5}' | head -n 1)

# 2. Verificar que se encontró una interfaz antes de realizar cambios
if [ -n "$UPNP_IFACE" ]; then
    echo "Interfaz de enlace ascendente UPnP activa encontrada: $UPNP_IFACE"
    if ! grep -q "# === Custom UPnP Network & Renderer Parameters ===" /etc/upmpdcli.conf; then
        echo "Aplicando configuración UPnP personalizada..."
        cat <<EOT | sudo tee -a /etc/upmpdcli.conf

# === Custom UPnP Network & Renderer Parameters ===

# Interfaz(ces) de red para usar con UPnP (descubierta dinámicamente)
upnpiface = ${UPNP_IFACE}

# Parámetros del renderizador de medios
# "Nombre amigable" para el renderizador de medios.
friendlyname = UpMpd-%h

# Nombre amigable específico para el renderizador de medios UPnP/AV.
avfriendlyname = Diretta
EOT
    else
        echo "La configuración ya existe en /etc/upmpdcli.conf. Actualizando el nombre de la interfaz si ha cambiado..."
        sudo sed -i "s/^upnpiface[[:space:]]*=[[:space:]]*.*/upnpiface = ${UPNP_IFACE}/" /etc/upmpdcli.conf
    fi
else
    echo "ERROR: No se pudo determinar mediante programación la interfaz de enlace ascendente." >&2
fi
```
...
>
> Si tiene UPnP instalado y habilitado, ahora es un buen momento para regresar al [**Apéndice 5**](#18-appendix-5-system-health-checks) y ejecutar el comando universal **System Health Check** (Comprobación de estado del sistema) tanto en el Host como en el Target.
>
> ---

---

## 11. Opción de integración de NAA

**Objective:** Habilite el protocolo Signalyst HQPlayer NAA (Adaptador de Audio de Red) en el Host Diretta.

---

## 12. Opción de integración de LMS

**Objective:** Habilite el protocolo de transmissão SlimProto para la integración con Lyrion Media Server en el Host Diretta

---

## 13. Opción de integración de AES

**Objective:** Habilite el protocolo de transmisión de baja latencia AES67 para compatibilidad com os serviços de áudio RAVENNA e Dante em o Host Diretta

---

## 14. Apéndice 1: Control de ventilador Argon ONE opcional
...
## 15. Apéndice 2: Control remoto IR opcional
...
**Nota:** Es normal que la parte `Installing Python-3.14.7...` tarde unos 10 minutos mientras compila Python desde el código fuente. ¡No se rinda! Siéntase libre de relajarse con hermosa música utilizando su nuevo sistema Diretta mientras espera. Debería estar disponible mientras Python se instala en el Host.

---

### **Paso 3: Descargar el repositorio del software `roon-ir-remote`**

Clone el repositorio del script o actualícelo si ya tiene una copia.
...
>
> El hardware y el software de su control remoto IR ahora deberían estar configurados. Para verificar la configuración, proceda al [**Apéndice 5**](#18-appendix-5-system-health-checks) y ejecute el comando universal **System Health Check** (Comprobación de estado del sistema) en el Diretta Host.
>
> ---
...
## 16. Apéndice 3: Modo purista opcional
...
menu_wrapper() {
  ...
  if [ "$was_active" = true ]; then
    ...
    purist-mode --revert > /dev/null 2>&1 # Revertir silenciosamente
  fi
  ...
  if [ "$was_active" = true ]; then
    ...
    purist-mode > /dev/null 2>&1 # Activar silenciosamente
...
## 17. Apéndice 4: Interfaz web de control del sistema opcional
...
3.  **Crear scripts de comandos seguros:**
    Crearemos un conjunto de pequeños scripts dedicados que son las *únicas* acciones que la aplicación web tiene permitido realizar. Este es un paso de seguridad crítico.
    ```bash
    ...
    # Verificar el Modo Purista
    if [ -f "/etc/nsswitch.conf.purist-bak" ]; then
      IS_ACTIVE="true"
    fi

    # Verificar si el arranque automático está habilitado
    if systemctl is-enabled --quiet purist-mode-auto.service; then
      IS_AUTO_ENABLED="true"
    fi

    # Verificar la caché de arranque validada en busca de un enlace de evaluación activo
    if [ ! -f /run/diretta/license.cache ] || grep -q "http" /run/diretta/license.cache; then
      LICENSE_LIMITED="true"
    fi

    # Informar nuestra MTU para que el Host pueda señalar un enlace cuyos dos extremos no coinciden
    MTU=$(cat /sys/class/net/end0/mtu 2>/dev/null || echo 0)

    # Y la MTU propia de Diretta, que es la que decide si la carga útil de un
    # ciclo cabe en una sola trama. El paso 2 escribe ambas juntas, de modo que
    # normalmente coinciden, pero nada lo impone después, y cuando divergen el
    # Host informaría de un ciclo de la mitad de su duración real.
    ...
    # Mostrar todos los indicadores de estado como un único objeto JSON
    ...
    #!/bin/bash
    if [[ "$1" == "--enforce" ]]; then
        # Aplicación absoluta: si se supone que debe estar activo, volver a ejecutar
        # el script base para limpiar cualquier ruta predeterminada resucitada.
        if [ -f "/etc/nsswitch.conf.purist-bak" ]; then
    ...
    #!/bin/bash
    # Reinicia el servicio Diretta ALSA Target.
    # Este script está pensado para ser invocado mediante sudo por el usuario purist-app.
    ...
    # Crear el script para reiniciar o apagar esta máquina
    cat <<'EOT' | sudo tee /usr/local/bin/pm-power
    #!/bin/bash
    # Reinicia o apaga esta máquina en nombre de la interfaz web.
    # El comando se desacopla para que quien lo invoca retorne de inmediato: en el Target
    # la sesión SSH se cierra limpiamente, y en el Host el navegador recibe su
    # confirmación antes de que la máquina se apague.
...
4.  **Otorgar permisos de Sudo:**
    Este paso permite al usuario `purist-app` ejecutar esos scripts con privilegios de root y sin necesidad de una terminal interactiva.
    ```bash
    cat <<'EOT' | sudo tee /etc/sudoers.d/purist-app
    # Indicar a sudo que no requiera un TTY para el usuario purist-app
    Defaults:purist-app !requiretty

    # Permitir al usuario purist-app ejecutar los scripts de control específicos sin contraseña
...
5.  **Rellenar el archivo de caché de la licencia de Diretta en el arranque**
    ...
    Type=oneshot
    RemainAfterExit=yes
    # systemd crea /run/diretta y lo mantiene entre reinicios de la unidad
    RuntimeDirectory=diretta
    RuntimeDirectoryMode=0755
    RuntimeDirectoryPreserve=yes
    # Bloquear limpiamente la ejecución aquí hasta que el Host responda a un ping
    TimeoutStartSec=infinity
...
10. **Instalar el script de encendido y otorgar permisos de Sudo en el Host:**
    El Host necesita el mismo script `pm-power` que el Target, para que la interfaz web pueda apagar ambas mitades de la pareja. Créelo también aquí:
    ```bash
    cat <<'EOT' | sudo tee /usr/local/bin/pm-power
    #!/bin/bash
    # Reinicia o apaga esta máquina en nombre de la interfaz web.
    ...
    Luego otorgue los permisos de sudo. Este paso es crítico para permitir que la aplicación web reinicie los servicios de Roon necesarios, y que reinicie o apague el Host, sin una contraseña.
    ```bash
    cat <<'EOT' | sudo tee /etc/sudoers.d/webui-restarts
    # Permitir que la webui (ejecutándose como audiolinux) aplique los perfiles del host y reinicie servicios
...
* **Home:** La página de inicio principal con enlaces a las distintas aplicaciones. También incluye el panel **Point-to-Point Link**, que muestra la velocidad de enlace negociada y la MTU, los periodos `CycleTime` e `InfoCycle` definidos actualmente en `setting.inf`, y los formatos DSD y PCM más altos que esos valores permiten. Mientras suena la música también mide el propio enlace y muestra lo que encuentra bajo cada valor configurado: el ciclo en el que el Host transmite realmente, el intervalo al que el Target informa realmente, el tamaño medio de trama y la porción del cable en uso. Un valor medido lleva una `~` durante sus primeros segundos, mientras sigue siendo una aproximación y no un hallazgo. Al pasar el cursor sobre cualquier celda en un navegador de escritorio se explica qué significa ese valor. Las cifras de formato se derivan de la regla de una sola transmisión por ciclo descrita en [**Apéndice 9**](#22-apéndice-9-optimización-de-tramas-jumbo-opcional), por lo que cambian a medida que avanza por los niveles de tramas jumbo, y los dos periodos de ciclo vuelven a cambiar entre los modos Purist y Super Purist. Dos fallos de MTU se señalan en rojo cuando el panel tiene pruebas de ellos: que el Host y el Target discrepen sobre la MTU de la interfaz — el fallo silencioso en el que el enlace se establece pero se descarta cada trama de tamaño completo — y que la `EtherMTU` del propio Target difiera de su propia interfaz, lo que deja a Diretta formando tramas más pequeñas de lo que suponen todas las cifras anteriores. Ambos necesitan una visita a la pestaña **Purist Mode App** para comprobarse, porque ninguna otra página pregunta nada al Target; hasta entonces el panel da por hecho que los dos extremos coinciden, como así será salvo que haya cambiado uno de ellos por separado. Mientras el Diretta Target no esté licenciado, la página también muestra una tarjeta **License Activation Required** (Se requiere activación de licencia) con el enlace de compra propio de ese Target y un botón **Restart Services** (Reiniciar servicios) que pulsar una vez completada la compra; la tarjeta desaparece en cuanto el Target se declara licenciado. Mientras esté ahí, todo lo que suene por encima del techo de 48 kHz del periodo de prueba — es decir, cualquier frecuencia DSD nativa — se muestra en amarillo bajo las cifras de formato, porque el Target lo detendrá a los seis minutos.

* **Botón de encendido:** El botón rojo de encendido en la esquina superior derecha de cada página ofrece **Reboot System** y **Power Off System**. Ambos actúan sobre el par, apagando primero el Target y unos segundos después el Host, ya que solo se puede acceder al Target a través del Host. Cada opción pide confirmación antes de hacer nada. Tras un apagado, ambas máquinas deben encenderse a mano.

* **Purist Mode App:** Esta página contiene los controles para alternar el Modo Purista y su comportamiento de inicio automático en el Diretta Target. Se actualiza automáticamente cada 30 segundos para mostrar el estado actual.
...
>
> La interfaz web del Modo Purista ahora debería estar operativa. Para verificar todos los componentes de esta función compleja, proceda al [**Apéndice 5**](#18-appendix-5-system-health-checks) y ejecute el comando universal **System Health Check** (Comprobación de estado del sistema) tanto en el Host como en el Target.
>
> ---

## 18. Apéndice 5: Comprobaciones de estado del sistema
...
## 19. Apéndice 6: Ajuste de rendimiento en tiempo real opcional
...
## 20. Apéndice 7: Optimizaciones de IRQ e hilos opcionales
...
2.  Inicie la reproducción de música en el Target Diretta. Esto asegura que el script pueda detectar el tráfico de interrupción activo. Si no tiene nada a mano, puede ejecutar este comando en el Host:
    ```bash
...
>
> Su ajuste avanzado en tiempo real ahora debería estar completo. Para verificar todos los componentes de esta nueva configuración, regrese al [**Apéndice 5**](#18-appendix-5-system-health-checks) y ejecute el comando universal **System Health Check** (Comprobación de estado del sistema) tanto en el Host como en el Target.
>
> ---

## 21. Apéndice 8: Velocidades de red puristas opcionales
...
>
> Su enlace de red dedicado ahora está configurado para la operación "Purista" de 100 Mbps. Para verificar que el servicio del Host esté activo y que el Target haya negociado correctamente la velocidad (detectada a través del archivo marcador), regrese al [**Apéndice 5**](#18-appendix-5-system-health-checks) y ejecute el comando universal **System Health Check** (Comprobación de estado del sistema) tanto en el Host como en el Target.
>
> ---

## 22. Apéndice 9: Optimización de tramas jumbo opcional
Este sección optimiza el transporte para alta eficiencia de ancho de banda.

#### **Paso 1:** Preparar interfaces

Debemos forzar temporalmente las interfaces de red a la mayor MTU jumbo que acepte este kernel, para verificar la compatibilidad del kernel y preparar la prueba de enlace. El bloque prueba **10222** y recurre a **9000**; un controlador rechaza de plano una MTU superior a su propio techo, sin cambiar nada, así que probar primero la mayor no cuesta nada.
...
case "$end0_mtu" in
  10222) echo "ÉXITO: Este kernel admite MTU 10222. Continúe con el paso 2." ;;
  9000)  echo "ÉXITO: Este kernel admite MTU 9000. Continúe con el paso 2." ;;
  *)     echo "ALTO: Este kernel no parece admitir ninguna MTU jumbo." ;;
esac
```

*Si ve "ALTO" en **cualquiera** de los dos (Host o Target), no proceda. Su kernel no tiene el parche requerido.*

*Las dos máquinas no tienen por qué coincidir aquí. Este paso informa de lo que aceptará cada **kernel**, y un Pi 5 alcanza 10222 donde un Pi 4 con un kernel más antiguo se detiene en 9000. Los pasos 2 y 3 fijan la pareja en un único nivel probando el enlace en sí.*
...
```bash
# 1. Detectar la mayor MTU que este enlace transportará realmente
...
EOF
  ...
  if [ "$NEW_MTU" -eq 10222 ]; then
    echo "Optimización: MTU 10222. Se mantiene CycleTime en 1500us."
    ...
    echo "Optimización: MTU 9000. Se relaja CycleTime a 1500us."
    sudo sed -i 's/^CycleTime=.*/CycleTime=1500/' /opt/diretta-alsa/setting.inf
    sudo sed -i 's/^InfoCycle=.*/InfoCycle=150000/' /opt/diretta-alsa/setting.inf
  elif [ "$NEW_MTU" -eq 3824 ]; then
    echo "Optimización: MTU 3824. Se relaja CycleTime a 1300us."
...
Reinicie primero el Target y después el Host:
```bash
sudo sync && sudo reboot
```

*Con esto finaliza la configuración. Las placas tardan uno o dos minutos en volver; lo que sigue explica de dónde salen estas cifras y no le pide nada.*
...
>
> **Por qué 10222 no relaja más el `CycleTime`.** Todo nivel hasta 9000 gasta sus bytes adicionales en un ciclo más largo y silencioso. Este los gasta en carga útil y mantiene el ciclo en 1500 µs. Lo que un ciclo puede transportar es `(MTU - 2) / CycleTime` bytes por µs: **5.9987** con MTU 9000 y **6.8133** con 10222. El estéreo de 32 bits a 768 kHz necesita **6.1440**, de modo que a 9000 se queda a 218 bytes de la transmisión única por ciclo y se envía como dos tramas; a 10222 cabe con margen. Nada más en esta plataforma pide más — DSD512 y 2xDXD necesitan ambos 5.6448 y ya caben a 9000 —, así que este nivel existe para exactamente un formato, y solo porque dividirlo en dos tramas es la fragmentación que estos niveles se crearon para evitar.
>
> **10222 es un techo de hardware, no una cifra redonda.** El controlador `macb` de la Raspberry Pi 5 tiene un búfer de recepción de 10240 bytes; reste los 14 bytes de cabecera Ethernet y los 4 bytes de FCS y quedan 10222. Es por tanto la mayor MTU que una Pi 5 aceptará siquiera, y así el valor más alto en el que puede convenir cualquier pareja que incluya una. Una Pi 4 con un kernel actual anuncia bastante más — 16347 — pero esta guía declina ese margen deliberadamente: alcanza una banda en la que las tramas sobredimensionadas se corrompen en silencio en lugar de ser rechazadas, y no compra ningún formato que esta plataforma pueda reproducir. 10222 es el techo que este proyecto admite hasta que un kernel demuestre ser estable por encima en **ambas** placas.
>
...
>
> Estos niveles existen porque esas eran las mayores MTU que el kernel entonces vigente transportaba en una Pi 4, no porque la placa no pudiera dar más de sí. El kernel de tiempo real AudioLinux LTS incorpora ahora un parche que da **9000** al mismo hardware, sin cambiar el bootloader ni la EEPROM, de modo que la escalera de los pasos 2 y 3 encuentra 9000 donde antes se detenía en 3824. Fíjese, eso sí, en qué kernel instala: el parche ha llegado de forma desigual. Al momento de escribir esto, el kernel LTS (`6.18.50-2`) lleva una Pi 4 más allá de 9000 — anuncia 16347, de los cuales esta guía usa 10222 — mientras que el más reciente `7.1.8-1` solo lleva el parche de 2032, así que elegir *Audiolinux last RT LTO* en el actualizador de kernel le cuesta tres niveles a una Pi 4 y baja el `CycleTime` de 1500 µs a 700 µs. Una Pi 5 alcanza 10222 con cualquiera de ellos, porque su techo es una propiedad de la placa y no del parche. Por eso los niveles menores siguen documentados: un kernel parcheado en parte, o nada en absoluto, sigue deteniéndose donde siempre lo hizo.
>
...
> ### ✅ Punto de control: Verificar la configuración de red
>
> Si pudo habilitar el soporte de tramas Jumbo para su configuración, ahora es un buen momento para regresar al [**Apéndice 5**](#18-appendix-5-system-health-checks) y ejecutar el comando universal **System Health Check** (Comprobación de estado del sistema) tanto en el Host como en el Target.
>
> ---

## 23. Apéndice 10: Actualizaciones del sistema opcionales
...
>    purist-mode --revert
>    ```
> 3. Ejecute el script universal de control de calidad **System Health Check** (Comprobación de estado del sistema) del [**Apéndice 5**](#18-appendix-5-system-health-checks) tanto en el Host como en el Target.
