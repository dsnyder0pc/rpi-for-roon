...
Diese Anweisungen wurden zuletzt mit **AudioLinux V5** getestet (Image: `audiolinux_pi4-pi5_530`, Menüversion: `544`).
...
* 1 x [Kurzes CAT6 Ethernet-Patchkabel](https://www.amazon.com/Cable-Matters-Snagless-Ethernet-Internet/dp/B0B57S1G2Y) (für die Punkt-zu-Punkt-Verbindung)
...
* 1 x [Stromverteilerkabel](https://www.amazon.com/dp/B01K3ADXX2) (beide 45W-Netzteile in eine einzige Steckdose stecken)
...
Die Stückliste dieses Leitfadens verwendet einen Raspberry Pi 5 sowohl für das Diretta-Target (das Gerät, das mit Ihrem DAC verbunden ist) als auch für den Diretta-Host, doch die beiden Rollen sind hardwareunabhängig: Pi 4 zu Pi 4, Pi 4 zu Pi 5, Pi 5 zu Pi 4 und Pi 5 zu Pi 5 werden alle gleichermaßen unterstützt, und jede Kombination erreicht dieselbe MTU-10222-Verbindung. Der einzige Unterschied: Ein Pi-4-Target verzichtet auf die USB-Interrupt-Steuerung aus [**Anhang 7**](#20-appendix-7-optional-irq-and-thread-optimizations), die die hardwareseitig festgelegten Interrupts dieses Boards nicht zulassen. Compute-Module sind ungetestet. Sie werden zuerst den Host konfigurieren.
...
echo 'Erinnerung: Das Standardpasswort befindet sich in Ihrer AudioLinux-E-Mail von Piero'
...
cmd=$(cat <<'EOT'
clear
echo "Willkommen zur interaktiven Einrichtung der Zeitzone."
...
echo
echo "Zeitzone wird auf ${timezone} eingestellt..."
sudo timedatectl set-timezone "$timezone"
echo "✅ Zeitzone wurde eingestellt."
...
3.  Wählen Sie auf dem nächsten Bildschirm **UPDATE system**, folgen Sie den Anweisungen und warten Sie, bis der Vorgang abgeschlossen ist.
...
5.  Wählen Sie **SELECT/Update kernel**. Wenn sich die Version neben „Your kernel“ von der nach „Audiolinux LTS RT LTO“ aufgeführten Version unterscheidet, wählen Sie Option 1, **Audiolinux LTS RT LTO (default)**, um den Kernel zu aktualisieren.
6.  Verlassen Sie das Menüsystem, um zum Terminal zurückzukehren.
...
```bash
cmd=$(cat <<'EOT'
read -rp "Geben Sie die endgültige Adresse Ihres Diretta-Hosts ein und drücken Sie [Enter]: " RPi_IP_Address
echo '$' ssh "audiolinux@${RPi_IP_Address}"
ssh -o StrictHostKeyChecking=accept-new "audiolinux@${RPi_IP_Address}"
EOT
)
...
if ! grep -q "AddKeysToAgent yes" "$SSH_CONFIG_FILE"; then
  echo "✅ Empfohlene globale SSH-Einstellungen werden hinzugefügt..."
  ...
  echo "✅ Empfohlene globale SSH-Einstellungen sind bereits vorhanden. Keine Änderungen vorgenommen."
fi

# --- Diretta-spezifische Host-Konfigurationen hinzufügen ---
if grep -q "Host diretta-host" "$SSH_CONFIG_FILE"; then
  echo "✅ SSH-Konfiguration für 'diretta-host' ist bereits vorhanden. Keine Änderungen vorgenommen."
else
...
echo ""
echo "--- Ihre ~/.ssh/config-Datei enthält jetzt: ---"
cat "$SSH_CONFIG_FILE"
EOT
)
bash -c "$cmd"
...
### 7.7. Neustart
Starten Sie neu (zuerst das Target, dann den Host), um den Status der Shadow-Dienste zu aktualisieren:
```bash
sudo sync && sudo reboot
```
...
7.  Sie sollten diese Aktionen nacheinander ausführen:
    ...
    * Wählen Sie **6) License**. Das System spielt hochauflösendes Audio (mehr als 48 kHz PCM) 6 Minuten lang im Testmodus ab. Folgen Sie dem Link auf dem Bildschirm und den Anweisungen, um Ihre vollständige Lizenz für Hi-Res-Unterstützung zu erwerben und anzuwenden. Dies erfordert den in Schritt 5 konfigurierten Internetzugang.
        ```text
...
1.  Führen Sie `menu` auf dem Host aus, falls Sie nach dem vorherigen Schritt zum Terminal zurückgekehrt sind, andernfalls gehen Sie zum **Main menu**.
...
> ---
> ### ✅ Checkpoint: Roon-Integration überprüfen
>
> Wählen Sie Ihre neue Diretta-Zone in der Roon-Steuerungs-App aus und spielen Sie etwas Musik ab.
>
> Jetzt ist ein guter Zeitpunkt, zu [**Anhang 5**](#18-appendix-5-system-health-checks) zurückzukehren und den universellen Befehl **System-Gesundheitscheck** sowohl auf dem Host als auch auf dem Target auszuführen, um die Konfiguration zu überprüfen.
>
> ---
    ...
    Sie müssen das Hardware-Overlay für den IR-Empfänger des Argon-One-Gehäuses aktivieren.
      ...
      * Dieser Befehl fügt das erforderliche Hardware-Overlay sicher zu Ihrer Datei `/boot/config.txt` hinzu und prüft vorher, ob es nicht bereits vorhanden ist.
        ...
        if ! sed 's/#.*//' $BOOT_CONFIG | grep -q -F "$IR_CONFIG"; then
          echo "Argon One IR-Empfänger wird aktiviert..."
          sudo sed -i "/# Uncomment this to enable infrared communication./a $IR_CONFIG" /boot/config.txt
        else
          echo "Argon One IR-Empfänger ist bereits aktiviert."
        fi
        ```
...
echo ""
echo ""
echo "Ihre Gruppenmitgliedschaften werden überprüft:"
echo "\$ groups"
groups
echo ""
...
if [ ! -d "$HOME/.pyenv" ]; then
  echo "--- pyenv wird installiert ---"
  curl -fsSL https://pyenv.run | bash
else
  echo "--- pyenv ist bereits installiert. Installation wird übersprungen. ---"
fi
...
if ! pyenv versions --bare | grep -q "^${PYVER}$"; then
    ...
    if [ "$TOTAL_MEM" -lt 1900 ]; then
        echo "--- Physischer RAM beträgt ${TOTAL_MEM}MB. Begrenzung auf 1 Kern, um ein Einfrieren zu verhindern. ---"
        export MAKE_OPTS="-j1"
        export MAKEFLAGS="-j1"
        mkdir -p "$HOME/pyenv_build_scratch"
        export TMPDIR="$HOME/pyenv_build_scratch"
    else
        echo "--- Physischer RAM beträgt ${TOTAL_MEM}MB. Paralleler Build wird fortgesetzt. ---"
    fi

    echo "--- Python ${PYVER} wird installiert. Dies wird einige Minuten dauern... ---"
    pyenv install "$PYVER"
    [ -n "$TMPDIR" ] && [ -d "$TMPDIR" ] && rm -rf "$TMPDIR"
else
    echo "--- Python ${PYVER} ist bereits installiert. ---"
fi

pyenv global "$PYVER"
```

**Hinweis:** Es ist normal, dass der Teil `Installing Python-3.14.7...` etwa 10 Minuten dauert, da Python aus den Quellen kompiliert wird. Geben Sie nicht auf! Entspannen Sie sich bei schöner Musik über Ihre neue Diretta-Zone in Roon, während Sie warten. Diese sollte verfügbar sein, während Python auf dem Host installiert wird.
...
echo "WICHTIG: Dies muss genau mit dem Zonennamen in der Roon-App übereinstimmen (Groß-/Kleinschreibung beachten)."
# Diese Zeile ist der Fix: < /dev/tty weist read an, das Terminal zu verwenden
read -rp "Geben Sie den Namen Ihrer Roon-Zone ein: " MY_ROON_ZONE < /dev/tty
...
if [ -f "/etc/systemd/system/ir-keymap.service" ]; then
    ...
    echo "--- Standard-IR-Empfänger erkannt. KEY_VOLUMEUP/DOWN wird verwendet. ---"
else
    VOL_UP_CODE="KEY_UP"
    VOL_DOWN_CODE="KEY_DOWN"
    echo "--- Flirc/HID-Adapter erkannt. KEY_UP/DOWN wird für die Lautstärke verwendet. ---"
fi
...
echo ""
echo "✅ Konfigurationsdatei 'roon-ir-remote/app_info.json' wurde erfolgreich erstellt."
...
if ! pyenv versions --bare | grep -q "^roon-ir-remote$"; then
  echo "--- Virtuelle Umgebung 'roon-ir-remote' wird erstellt ---"
  pyenv virtualenv roon-ir-remote
else
  echo "--- Virtuelle Umgebung 'roon-ir-remote' existiert bereits ---"
fi
...
```bash
echo ""
echo "- Purist-Modus wird deaktiviert, um einen sauberen Zustand zu gewährleisten"
purist-mode --revert

echo ""
echo "- Dienst zur Rückkehr in den Standardmodus bei jedem Systemstart wird erstellt"
...
echo ""
echo "- Dienst für die verzögerte automatische Aktivierung wird erstellt"
...
echo ""
echo "- Neue Dienste werden aktiviert"
sudo systemctl daemon-reload
...
if grep -q menu_wrapper ~/.bashrc; then
  ...
  echo ""
  echo "Wrapper um den menu-Befehl hinzufügen"
...
menu_wrapper() {
  ...
  if [ "$was_active" = true ]; then
    echo "Berechtigungen zur Verwaltung des Purist-Modus werden überprüft..."
    sudo -v

    echo "Purist-Modus wird vorübergehend deaktiviert, um das Menü auszuführen..."
  ...
  if [ "$was_active" = true ]; then
    echo "Purist-Modus wird reaktiviert..."
...
alias purist-mode-auto-enable='echo "Purist-Modus wird beim Systemstart aktiviert..."; purist-mode; sudo systemctl enable purist-mode-auto.service'
alias purist-mode-auto-disable='echo "Purist-Modus wird beim Systemstart deaktiviert..."; purist-mode --revert; sudo systemctl disable --now purist-mode-auto.service'
EOT
fi
...
2.  **Optionale Auto-Aktivierung:** Wenn Sie die automatische Funktion aktiviert haben, wartet das System nach dem Booten bis zu 60 Sekunden und schaltet dann automatisch in den **Purist-Modus**. Dies bietet ein komfortables Benutzererlebnis für diejenigen, die immer im optimierten Zustand hören möchten.
...
Sie haben jederzeit die volle interaktive Kontrolle über das System.
  ...
  * Um das **automatische Boot-Verhalten** zu steuern, verwenden Sie die praktischen Aliase auf dem Diretta-Target:
    ...
    ```bash
    # Dies aktiviert die automatische Aktivierung nach dem Stellen der Uhr beim nächsten Booten
...
3.  **Sichere Befehlsskripte erstellen:**
    Wir werden eine Reihe kleiner, dedizierter Skripte erstellen. Dies sind die *einzigen* Aktionen, die die Web-App ausführen darf. Dies ist ein wichtiger Sicherheitsschritt.
    ```bash
    ...
    # Unsere MTU melden, damit der Host eine Verbindung erkennt, deren beide Enden nicht übereinstimmen
    MTU=$(cat /sys/class/net/end0/mtu 2>/dev/null || echo 0)

    # Und Direttas eigene MTU, die darüber entscheidet, ob die Nutzlast eines
    # Zyklus in einen einzigen Frame passt. Schritt 2 schreibt beide zusammen,
    # daher stimmen sie normalerweise überein, doch danach erzwingt das nichts,
    # und driften sie auseinander, meldete der Host einen halb so langen Zyklus.
    ...
    if [ -s "$CACHE_FILE" ]; then
        ...
        echo "Fehler: Lizenz-Cache nicht gefunden oder leer." >&2
        exit 1
    fi
    EOT
    ...
    if [ "$SPEED" = "10" ]; then
        echo "10-Mbit/s-Begrenzung (Super Purist) wird eingeplant..."
        /usr/bin/sh -c "sleep 1 && sudo /usr/bin/ethtool -s end0 advertise 0x002" >/dev/null 2>&1 < /dev/null &
    elif [ "$SPEED" = "100" ]; then
        echo "100-Mbit/s-Begrenzung (Purist) wird eingeplant..."
    ...
    # Skript zum Neustarten oder Ausschalten dieser Maschine erstellen
    cat <<'EOT' | sudo tee /usr/local/bin/pm-power
    #!/bin/bash
    # Startet diese Maschine im Auftrag der Web-UI neu oder schaltet sie aus.
    # Der Befehl wird abgekoppelt, sodass der Aufrufer sofort zurückkehrt: auf dem Target
    # schließt die SSH-Sitzung sauber, und auf dem Host erhält der Browser seine
    # Bestätigung, bevor die Maschine heruntergefahren wird.
...
4.  **Sudo-Berechtigungen erteilen:**
    Dieser Schritt ermöglicht es dem Benutzer `purist-app`, diese Skripte mit Root-Rechten und ohne die Notwendigkeit eines interaktiven Terminals auszuführen.
    ```bash
...
3.  **SSH konfigurieren und den Schlüssel auf das Target kopieren:**
    ...
    echo "--> Öffentlicher Schlüssel wird auf das Target kopiert..."
...
4.  **Den Schlüssel auf dem Target autorisieren:**
    ...
    ```bash
    echo "--> Setup-Skript wird auf dem Target ausgeführt..."
    set -e
    ...
    echo "✅ SSH-Schlüssel wurde erfolgreich auf dem Target autorisiert."
    ```
...
7.  **Avahi und Python-Abhängigkeiten auf dem Diretta-Host installieren:**
    ...
    **Hinweis:** OPTIONAL – Wenn Sie mehr als einen Diretta-Host in Ihrem Netzwerk haben, stellen Sie bitte sicher, dass diese eindeutige Namen besitzen. Sie können einen Befehl wie den folgenden verwenden, um diesen umzubenennen, bevor Sie fortfahren:
    ...
    echo "--- Avahi wird für die Verwendung der Schnittstelle konfiguriert: $USB_INTERFACE ---"
    ...
    echo "--- Python-Umgebung für das Web-UI wird eingerichtet ---"
    # Die virtuelle Umgebung nur erstellen, falls sie noch nicht existiert
    if ! pyenv versions --bare | grep -q "^purist-webui$"; then
      echo "--- Virtuelle Umgebung 'purist-webui' wird erstellt ---"
      pyenv virtualenv purist-webui
    else
      echo "--- Virtuelle Umgebung 'purist-webui' existiert bereits ---"
    fi
...
9. **Port-Binding-Berechtigung erteilen**
    ...
    echo "Capability wird auf die reale Datei angewendet: ${PYTHON_EXEC}"
    sudo setcap 'cap_net_bind_service=+ep' "$PYTHON_EXEC"
    ```

10. **Das Power-Skript installieren und Sudo-Berechtigungen auf dem Host erteilen:**
    Der Host benötigt dasselbe `pm-power`-Skript wie das Target, damit die Web-UI beide Hälften des Gespanns herunterfahren kann. Erstellen Sie es also auch hier:
    ```bash
    cat <<'EOT' | sudo tee /usr/local/bin/pm-power
    #!/bin/bash
    # Startet diese Maschine im Auftrag der Web-UI neu oder schaltet sie aus.
    ...
    Erteilen Sie anschließend die Sudo-Berechtigungen. Dieser Schritt ist entscheidend, damit die Webanwendung die erforderlichen Roon-Dienste neu starten und den Host ohne Passwort neu starten oder ausschalten kann.
    ```bash
...
* **Home:** Die Hauptstartseite mit Links zu den verschiedenen Anwendungen. Sie enthält außerdem das Bedienfeld **Point-to-Point Link**, das die ausgehandelte Verbindungsgeschwindigkeit und die MTU anzeigt, die derzeit in `setting.inf` gesetzten Perioden `CycleTime` und `InfoCycle` sowie die höchsten DSD- und PCM-Formate, die diese Werte zulassen. Während der Wiedergabe misst es zusätzlich die Verbindung selbst und zeigt unter jedem konfigurierten Wert, was es vorfindet: den Zyklus, auf dem der Host tatsächlich sendet, das Intervall, in dem das Target tatsächlich meldet, die mittlere Framegröße und den genutzten Anteil der Leitung. Ein gemessener Wert trägt in seinen ersten Sekunden ein `~`, solange er noch eine Näherung und kein Befund ist. Wenn Sie in einem Desktop-Browser über eine Zelle fahren, wird erklärt, was der Wert bedeutet. Die Formatangaben leiten sich aus der in [**Anhang 9**](#22-anhang-9-optionale-jumbo-frames-optimierung) beschriebenen Regel einer Übertragung pro Zyklus ab, verschieben sich also, während Sie die Jumbo-Frames-Stufen durchlaufen, und die beiden Zyklusperioden ändern sich erneut zwischen Purist- und Super-Purist-Modus. Zwei MTU-Fehler werden rot hervorgehoben, sobald das Bedienfeld den Beleg dafür hat: wenn Host und Target sich über die Schnittstellen-MTU uneinig sind — der stille Ausfall, bei dem die Verbindung steht, aber jeder Frame voller Größe verworfen wird — und wenn die `EtherMTU` des Targets von seiner eigenen Schnittstelle abweicht, wodurch Diretta kleiner sendet, als sämtliche Werte darüber annehmen. Beides erfordert einen Besuch der Registerkarte **Purist-Modus-App**, denn keine andere Seite fragt das Target überhaupt etwas; bis dahin nimmt das Bedienfeld an, dass beide Enden übereinstimmen — was sie tun, sofern Sie nicht eines davon allein geändert haben. Solange das Diretta-Target nicht lizenziert ist, trägt die Seite außerdem eine Karte **License Activation Required** (Lizenzaktivierung erforderlich) mit dem eigenen Kauf-Link dieses Targets und einer Schaltfläche **Restart Services** (Dienste neu starten), die nach abgeschlossenem Kauf zu drücken ist; die Karte verschwindet, sobald das Target sich als lizenziert meldet. Solange sie zu sehen ist, wird alles, was oberhalb der 48-kHz-Grenze der Testphase abgespielt wird — also jede native DSD-Rate —, unter den Formatwerten gelb dargestellt, denn das Target bricht die Wiedergabe nach sechs Minuten ab.

* **Power-Button:** Der rote Power-Button oben rechts auf jeder Seite bietet **Reboot System** und **Power Off System**. Beide wirken auf das Gerätepaar und fahren zuerst das Target und wenige Sekunden später den Host herunter, da das Target nur über den Host erreichbar ist. Vor jeder Aktion wird eine Bestätigung verlangt. Nach dem Ausschalten müssen beide Geräte von Hand wieder eingeschaltet werden.

* **Purist-Modus-App:** Diese Seite enthält die Steuerelemente zum Umschalten des Purist-Modus und seines Autostart-Verhaltens auf dem Diretta-Target. Sie aktualisiert sich alle 30 Sekunden automatisch, um den aktuellen Status anzuzeigen.
...
# Jetzt die Hardware-Optimierung einstellen
...
echo "Synchronisierung mit dem physischen Link-Layer..."
for i in {1..60}; do
    if [ -f /sys/class/net/$INTERFACE/carrier ] && [ "$(cat /sys/class/net/$INTERFACE/carrier 2>/dev/null)" "==" "1" ]; then
        echo "Physischer Link-Layer nach $i Sekunden erkannt."
        break
    fi
    sleep 1
done
...
if [ -f "$FLAG_FILE" ]; then
    echo "Super Purist-Flag erkannt. Kündige 10 Mbit/s Vollduplex an..."
    /usr/bin/ethtool -s $INTERFACE advertise 0x002
else
    echo "Standard/Purist-Modus. Kündige bis zu 100 Mbit/s Vollduplex an..."
...
if grep -q "Raspberry Pi 4" /proc/device-tree/model 2>/dev/null; then
    echo "Raspberry Pi 4 erkannt. Zwingend erforderlicher Hardware-Neuaushandlungsimpuls wird ausgelöst..."
    /usr/bin/ethtool -r $INTERFACE
elif grep -q "Raspberry Pi 5" /proc/device-tree/model 2>/dev/null; then
    echo "Raspberry Pi 5 erkannt. Interner automatischer phylib-Impuls wird genutzt; manueller Reset wird übersprungen."
else
    /usr/bin/ethtool -r $INTERFACE || true
fi

echo "Verbindungsgeschwindigkeitsrichtlinie erfolgreich abgeschlossen."
EOT
...
Wir müssen die Netzwerkschnittstellen vorübergehend auf die größte Jumbo-MTU zwingen, die dieser Kernel akzeptiert, um die Kernel-Unterstützung zu prüfen und den Verbindungstest vorzubereiten. Der Block versucht **10222** und fällt auf **9000** zurück; ein Treiber verweigert eine MTU oberhalb seiner eigenen Obergrenze rundweg und ändert dabei nichts, weshalb der Versuch mit dem größeren Wert zuerst nichts kostet.
...
case "$end0_mtu" in
  10222) echo "ERFOLG: Dieser Kernel trägt MTU 10222. Weiter mit Schritt 2." ;;
  9000)  echo "ERFOLG: Dieser Kernel trägt MTU 9000. Weiter mit Schritt 2." ;;
  *)     echo "STOPP: Dieser Kernel scheint keine Jumbo-MTU zu tragen." ;;
esac
```
...
*Die beiden Maschinen müssen hier nicht übereinstimmen. Dieser Schritt meldet, was der jeweilige **Kernel** akzeptiert: Mit dem Kernel, den diese Images mitbringen, erreichen beide Boards 10222, während ein Pi 4 auf einem älteren Kernel bei 9000 oder darunter stehen bleibt. Die Schritte 2 und 3 legen das Paar durch einen Test der Verbindung selbst auf eine einzige Stufe fest.*
...
```bash
# 1. Die größte MTU ermitteln, die diese Verbindung tatsächlich trägt
echo "Verbindungsfähigkeit wird getestet..."
if ping -c 1 -w 1 -M "do" -s 10194 host &>/dev/null; then
  ...
  echo "FEHLGESCHLAGEN: Verbindung unterstützt keine Jumbo Frames. Zurücksetzen auf sichere Standardwerte."
...
EOF
  ...
  echo "Diretta-Target wird konfiguriert..."
  ...
  echo "FERTIG: Target-Optimierung abgeschlossen."
}
...
EOF
  ...
  # 3. Diretta-Konfiguration anwenden
  echo "Diretta-Host wird konfiguriert..."

  # CycleTime wirkt nur, wenn der Host sein eigenes Profil verwendet. Bleibt
  # TargetProfileLimitTime ungleich null, wählt Direttas automatisches
  ...
  if [ "$NEW_MTU" -eq 10222 ]; then
    echo "Optimierung: MTU 10222. CycleTime bleibt bei 1500us."
    ...
    echo "Optimierung: MTU 9000. CycleTime wird auf 1500us gelockert."
    sudo sed -i 's/^CycleTime=.*/CycleTime=1500/' /opt/diretta-alsa/setting.inf
    sudo sed -i 's/^InfoCycle=.*/InfoCycle=150000/' /opt/diretta-alsa/setting.inf
  elif [ "$NEW_MTU" -eq 3824 ]; then
    echo "Optimierung: MTU 3824. CycleTime wird auf 1300us gelockert."
  ...
  echo "FERTIG: Host-Optimierung abgeschlossen."
}
```

#### **Schritt 4:** Neustart, damit die MTU-Änderungen übernommen werden
Starten Sie zuerst das Target neu, dann den Host:
```bash
sudo sync && sudo reboot
```

*Damit ist die Einrichtung abgeschlossen. Die Boards brauchen ein bis zwei Minuten, um zurückzukommen; was folgt, erklärt, woher diese Zahlen stammen, und verlangt nichts von Ihnen.*
...
>
> **Warum 10222 die `CycleTime` nicht weiter lockert.** Jede Stufe bis 9000 gibt ihre zusätzlichen Bytes für einen längeren, ruhigeren Zyklus aus. Diese gibt sie stattdessen für Nutzlast aus und hält den Zyklus bei 1500 µs. Was ein Zyklus tragen kann, ist `(MTU - 2) / CycleTime` Byte pro µs: **5.9987** bei MTU 9000, **6.8133** bei 10222. Stereo 32 Bit 768 kHz braucht **6.1440** und verfehlt die eine Übertragung pro Zyklus bei 9000 daher um 218 Byte und wird als zwei Frames gesendet; bei 10222 passt es mit Reserve. Nichts sonst auf dieser Plattform verlangt mehr — DSD512 und 2xDXD brauchen beide 5.6448 und passen bereits bei 9000 — diese Stufe existiert also für genau ein Format, und nur deshalb, weil dessen Aufteilung auf zwei Frames genau die Fragmentierung ist, gegen die diese Stufen gebaut wurden.
>
> **10222 ist eine Hardware-Obergrenze, keine runde Zahl.** Der `macb`-Treiber des Raspberry Pi 5 hat einen Empfangspuffer von 10240 Byte; ziehen Sie die 14 Byte Ethernet-Header und die 4 Byte FCS ab, bleiben 10222. Das ist somit die größte MTU, die ein Pi 5 überhaupt akzeptiert, und damit der höchste Wert, auf den sich ein Paar mit einem solchen Board einigen kann. Ein Pi 4 auf einem aktuellen Kernel meldet erheblich mehr — 16347 — doch dieser Leitfaden verzichtet bewusst auf diesen Spielraum: Er reicht in einen Bereich, in dem übergroße Frames still beschädigt werden, statt abgewiesen zu werden, und er kauft kein Format, das diese Plattform abspielen kann. 10222 ist die Obergrenze, die dieses Projekt unterstützt, bis sich ein Kernel darüber auf **beiden** Boards als stabil erweist.
>
