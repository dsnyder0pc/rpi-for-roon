...
9.  [Opção de Integração do Roon](#9-opção-de-integração-do-roon)
10. [Opção de Integração UPnP](#10-opção-de-integração-upnp)
11. [Opção de Integração NAA](#11-opção-de-integração-naa)
12. [Opção de Integração LMS](#12-opção-de-integração-lms)
13. [Opção de Integração AES](#13-opção-de-integração-aes)
14. [Apêndice 1: Controle de Ventoinha Argon ONE Opcional](#14-apêndice-1-controle-de-ventoinha-argon-one-opcional)
15. [Apêndice 2: Controle Remoto IR Opcional](#15-apêndice-2-controle-remoto-ir-opcional)
16. [Apêndice 3: Modo Purista Opcional](#16-apêndice-3-modo-purista-opcional)
17. [Apêndice 4: Interface Web Opcional de Controle do Sistema](#17-apêndice-4-interface-web-opcional-de-controle-do-sistema)
18. [Apêndice 5: Verificações de Saúde do Sistema](#18-apêndice-5-verificações-de-saúde-do-sistema)
19. [Apêndice 6: Ajuste Opcional de Desempenho em Tempo Real](#19-apêndice-6-ajuste-opcional-de-desempenho-em-tempo-real)
20. [Apêndice 7: Otimizações Opcionais de IRQ e Threads](#20-apêndice-7-otimizações-opcionais-de-irq-e-threads)
21. [Apêndice 8: Velocidades Opcionais de Rede Purista](#21-apêndice-8-velocidades-opcionais-de-rede-purista)
22. [Apêndice 9: Otimização de Jumbo Frames Opcional](#22-apêndice-9-otimização-de-jumbo-frames-opcional)
23. [Apêndice 10: Atualizações do Sistema Opcionais](#23-apêndice-10-atualizações-do-sistema-opcionais)
...
A lista de materiais deste guia usa um Raspberry Pi 5 tanto para o Diretta Target (o dispositivo conectado ao seu DAC) quanto para o Diretta Host, mas os dois papéis são independentes do hardware: Pi 4 para Pi 4, Pi 4 para Pi 5, Pi 5 para Pi 4 e Pi 5 para Pi 5 são todos igualmente suportados, e cada combinação alcança o mesmo enlace com MTU 10222. A única diferença é que um Pi 4 como Target abre mão do direcionamento de interrupções USB do [**Apêndice 7**](#20-appendix-7-optional-irq-and-thread-optimizations), que as interrupções fixadas por hardware dessa placa não permitem. Os Compute Modules não foram testados. Você configurará o Host primeiro.
...
1.  Insira o cartão microSD no **primeiro** Raspberry Pi, conecte-o à sua rede e ligue-o. **Nota:** Se você estiver usando o gabinete Argon ONE, poderá ouvir ruído da ventoinha. Não se preocupe. Depois de concluir a configuração do Diretta, há instruções no [Apêndice 1](#14-appendix-1-optional-argon-one-fan-control) para lidar com o ruído da ventoinha.
...
7.  Você deve realizar estas ações em sequência:
    ...
    * Escolha **6) License**. O sistema reproduzirá áudio de alta resolução (superior a 48 kHz PCM) por 6 minutos em modo de teste. Siga o link e as instruções na tela para adquirir e aplicar sua licença completa para suporte a alta resolução. Isso requer o acesso à internet que configuramos na etapa 5.
        ```text
...
>
> Seu sistema core Diretta e Roon deve estar totalmente funcional agora. Para verificar todos os serviços e conexões, por favor, prossiga para o [**Apêndice 5**](#18-appendix-5-system-health-checks) e execute o comando universal de **Verificação de Saúde do Sistema** em ambos o Host e o Target.
>
> ---

---

## 9. Opção de Integração do Roon
...
## 10. Opção de Integração UPnP

**Objetivo:** Habilitar recursos de renderização de mídia UPnP/DLNA no Diretta Host usando MPD (Music Player Daemon) e UPMPDCLI, permitindo compatibilidade com pontos de controle e reprodutores a montante, como Audirvāna, JRiver Media Center, mconnect ou BubbleUPnP.

Esta configuração permite que o Diretta Host receba fluxos de rede UPnP padrão e os envie de forma limpa para a camada do driver ALSA da Diretta sincronizada para transmissão através do link ponto a ponto para o Target.

> ---
> ### ⚠️ Nota de topologia: Executar apenas no Host
>
> Todas as etapas de instalação e configuração detalhadas neste apêndice devem ser executadas exclusivamente no **Diretta Host**. O Diretta Target continua sendo um endpoint de protocolo minimalista e não requer ajustes para reprodução UPnP.
>
> ---

### Passo 1: Instalar e habilitar MPD e UPMPDCLI

1. Estabeleça uma conexão SSH e faça login no **Diretta Host**.
2. Inicie a ferramenta de configuração do AudioLinux executando:
   ```bash
   menu
   ```
3. Navegue até o menu **INSTALL/UPDATE**.
4. Selecione **INSTALL/UPDATE MPD** e permita a conclusão do script de instalação.
5. Retorne à tela de atualização e selecione **INSTALL/UPDATE UPMPDCLI**, permitindo que a configuração termine.
6. Retorne ao **Menu principal**, selecione o menu **Audio** e digite **SHOW audio service**.
7. Confirme se **mpd** e **upmpdcli** estão ativos. Se algum serviço estiver ausente do grupo habilitado, selecione **MPD enable/disable** ou **UPMPDCLI enable/disable** respectivamente para ativá-los.
8. Saia do sistema de menus para retornar ao shell do terminal.

### Passo 2: Configurar a saída de áudio do MPD

Para direcionar o fluxo de áudio decodificado do MPD para a tubulação de transporte da Diretta, adicione os parâmetros de saída ALSA personalizados ao final do arquivo de configuração do MPD:

```bash
if grep -q 'Custom Diretta ALSA Audio Output' /etc/mpd.conf; then
  echo "Saída de áudio ALSA Diretta já configurada"
else
...
### Passo 3: Configurar o renderizador UPMPDCLI e os parâmetros de rede

Atualize os parâmetros de configuração do UPMPDCLI para definir a configuração correta da interface de rede a montante e atribuir um identificador amigável para descoberta por seus aplicativos de controle:

```bash
# 1. Descobrir dinamicamente a interface de uplink LAN ativa (garantindo um único nome de interface)
UPNP_IFACE=$(ip route show default | awk '{print $5}' | head -n 1)

# 2. Verificar se uma interface foi encontrada antes de fazer alterações
if [ -n "$UPNP_IFACE" ]; then
    echo "Interface de uplink UPnP ativa encontrada: $UPNP_IFACE"
    if ! grep -q "# === Custom UPnP Network & Renderer Parameters ===" /etc/upmpdcli.conf; then
        echo "Aplicando configuração UPnP personalizada..."
        cat <<EOT | sudo tee -a /etc/upmpdcli.conf

# === Custom UPnP Network & Renderer Parameters ===

# Interface(s) de rede para usar com UPnP (descoberta dinamicamente)
upnpiface = ${UPNP_IFACE}

# Parâmetros do renderizador de mídia
# "Nome amigável" para o renderizador de mídia.
friendlyname = UpMpd-%h

# Nome amigável específico para o renderizador de mídia UPnP/AV.
avfriendlyname = Diretta
EOT
    else
        echo "A configuração já existe em /etc/upmpdcli.conf. Atualizando o nome da interface se alterado..."
        sudo sed -i "s/^upnpiface[[:space:]]*=[[:space:]]*.*/upnpiface = ${UPNP_IFACE}/" /etc/upmpdcli.conf
    fi
else
    echo "ERRO: Não foi possível determinar programaticamente a interface de uplink." >&2
fi
```

### Passo 4: Reiniciar serviços

Execute uma recarga do systemd e reinicie ambos os daemons em segundo plano para forçar o sistema a inicializar suas substituições de configuração:
...
> ---
> ### ✅ Ponto de controle: Verificar o funcionamento do UPnP
>
> Abra a plataforma de controle UPnP/DLNA escolhida em um dispositivo remoto conectado à rede. Seu sistema deve descobrir o endpoint, exibindo **DIRETTA** como uma zona de reprodução ativa e selecionável.
>
> Se você tiver o UPnP instalado e habilitado, agora é um bom momento para retornar ao [**Apêndice 5**](#18-appendix-5-system-health-checks) e executar o comando universal de **Verificação de Saúde do Sistema** tanto no Host quanto no Target.
>
> ---

---

## 11. Opção de Integração NAA

**Objective:** Habilite o protocolo Signalyst HQPlayer NAA (Network Audio Adapter) no Host Diretta.

---

## 12. Opção de Integração LMS

**Objective:** Habilite o protocolo de streaming SlimProto para integração com o Lyrion Media Server no Host Diretta

---

## 13. Opção de Integração AES

**Objective:** Habilite o protocolo de streaming de baixa latência AES67 para compatibilidade com os serviços de áudio RAVENNA e Dante no Host Diretta

---

## 14. Apêndice 1: Controle de Ventoinha Argon ONE Opcional
...
## 15. Apêndice 2: Controle Remoto IR Opcional
...
Crie um serviço para executar o script automaticamente em segundo plano. Ele está vinculado ao Roon Bridge, então o controle remoto inicia, para e reinicia junto com ele, e é totalmente ignorado em um Host onde o Roon Bridge não está instalado.
...
# O Roon Bridge precisa estar instalado e em execução: o controle remoto comanda uma Zona do Roon.
# É ignorado de forma limpa quando ele falta, e parado sempre que o Roon Bridge para.
...
# Ignora a inicialização (sem erro) quando o Roon Bridge está instalado mas não está em execução.
...
# Inicia o controle remoto novamente sempre que o Roon Bridge sobe. PartOf= só propaga
# paradas e reinicializações, e um Wants= apontando para uma unit ausente é inofensivo, então este
# drop-in é seguro mesmo em um Host onde o Roon Bridge ainda não está instalado.
...
>
> O hardware e o software do seu Controle Remoto IR devem estar configurados agora. Para verificar a configuração, prossiga para o [**Apêndice 5**](#18-appendix-5-system-health-checks) e execute o comando de **Verificação de Saúde do Sistema** universal no Host Diretta.
>
> ---
...
## 16. Apêndice 3: Modo Purista Opcional
...
2.  **Auto-Ativação Opcional:** Se você ativou o recurso automático, o sistema aguardará até 60 segundos após a inicialização e, em seguida, mudará automaticamente para o **Modo Purista**. Isso fornece uma experiência de "configurar e esquecer" para usuários que sempre preferem ouvir no estado otimizado.
...
Você tem controle interativo total sobre o sistema a qualquer momento.
  ...
  * Para controlar o **comportamento automático de inicialização**, use os aliases de conveniência no Diretta Target:
    ...
    ```bash
    # Isso ativa a auto-ativação após o acerto do relógio na próxima inicialização
...
## 17. Apêndice 4: Interface Web Opcional de Controle do Sistema
...
3.  **Criar Scripts de Comando Seguros:**
    Criaremos um conjunto de pequenos scripts dedicados que são as *únicas* ações que o aplicativo web tem permissão de realizar. Esta é uma etapa de segurança crítica.
    ```bash
    ...
    # Informar nossa MTU para que o Host possa sinalizar um enlace cujas duas pontas divergem
    MTU=$(cat /sys/class/net/end0/mtu 2>/dev/null || echo 0)

    # E a MTU do próprio Diretta, que é a que decide se a carga útil de um
    # ciclo cabe num único quadro. O passo 2 grava as duas em conjunto, pelo
    # que normalmente coincidem, mas nada o impõe depois, e quando divergem o
    # Host passaria a relatar um ciclo com metade da sua duração real.
    ...
    # Criar o script para reiniciar ou desligar esta máquina
    cat <<'EOT' | sudo tee /usr/local/bin/pm-power
    #!/bin/bash
    # Reinicia ou desliga esta máquina em nome da interface web.
    # O comando é desacoplado para que o chamador retorne imediatamente: no Target
    # a sessão SSH fecha corretamente, e no Host o navegador recebe sua
    # confirmação antes que a máquina desligue.
...
4.  **Conceder Permissões de Sudo:**
    Esta etapa permite que o usuário `purist-app` execute esses scripts com privilégios de root e sem a necessidade de um terminal interativo.
    ```bash
...
5.  **Preencher o Arquivo de Cache de Licença do Diretta no Momento da Inicialização**
    A busca pela URL de Licença do Diretta requer uma conexão com a Internet. Se tivermos o Modo Purista ativado por padrão, o Target nunca poderá buscar a URL. No entanto, no momento da inicialização, temos o Modo Purista desativado por até 60 segundos para acertar o relógio e verificar a ativação da Licença do Diretta. Podemos usar essa janela de tempo para buscar a URL também.
    ```bash
...
7.  **Instalar o Avahi e as Dependências do Python no Host Diretta:**
    ...
    **Nota: OPCIONAL** - Se você tiver mais de um Host Diretta em sua rede, certifique-se de que eles tenham nomes exclusivos. Você pode usar um comando como o seguinte para renomear este antes de prosseguir:
    ...
    # O Avahi publica o único nome ao qual a interface web responde, portanto reinicie-o se ele falhar
...
10. **Instalar o Script de Energia e Conceder Permissões de Sudo no Host:**
    O Host precisa do mesmo script `pm-power` que o Target, para que a interface web possa desligar as duas metades do par. Crie-o aqui também:
    ```bash
    cat <<'EOT' | sudo tee /usr/local/bin/pm-power
    #!/bin/bash
    # Reinicia ou desliga esta máquina em nome da interface web.
    ...
    Em seguida, conceda as permissões de sudo. Esta etapa é crítica para permitir que o aplicativo web reinicie os serviços do Roon necessários, e reinicie ou desligue o Host, sem uma senha.
    ```bash
...
* **Home:** A página inicial principal, com ligações para as diferentes aplicações. Inclui também o painel **Point-to-Point Link**, que indica a velocidade de ligação negociada e a MTU, os períodos `CycleTime` e `InfoCycle` atualmente definidos em `setting.inf` e os formatos DSD e PCM mais altos que esses valores suportam. Enquanto a música toca, mede também a própria ligação e mostra o que encontra por baixo de cada valor configurado: o ciclo em que o Host está realmente a transmitir, o intervalo a que o Target está realmente a relatar, o tamanho médio do quadro e a fração do cabo em uso. Um valor medido leva um `~` nos primeiros segundos, enquanto ainda é uma aproximação e não uma conclusão. Passar o cursor sobre qualquer célula num navegador de desktop explica o que aquele valor significa. Os números de formato derivam da regra de uma só transmissão por ciclo descrita no [**Apêndice 9**](#22-apêndice-9-otimização-de-jumbo-frames-opcional), pelo que se alteram à medida que percorre os patamares de jumbo frames, e os dois períodos de ciclo mudam novamente entre os modos Purist e Super Purist. Duas falhas de MTU são assinaladas a vermelho quando o painel tem provas delas: o Host e o Target discordarem quanto à MTU da interface — a falha silenciosa em que a ligação se estabelece mas cada quadro de tamanho completo é descartado — e a `EtherMTU` do próprio Target diferir da sua própria interface, o que deixa o Diretta a formar quadros menores do que todos os números acima pressupõem. Ambas exigem uma passagem pelo separador **Purist Mode App** para serem verificadas, porque nenhuma outra página pergunta seja o que for ao Target; até lá, o painel assume que as duas pontas concordam, como acontecerá a menos que tenha alterado apenas uma delas. Enquanto o Target Diretta não estiver licenciado, a página traz também um cartão **License Activation Required** (Ativação de licença necessária) com o link de compra próprio desse Target e um botão **Restart Services** (Reiniciar serviços) a premir assim que a compra estiver concluída; o cartão desaparece mal o Target se declare licenciado. Enquanto lá estiver, tudo o que toque acima do limite de 48 kHz do período de avaliação — ou seja, qualquer frequência DSD nativa — aparece a amarelo por baixo dos números de formato, porque o Target o interromperá ao fim de seis minutos.

* **Botão de energia:** O botão vermelho de energia no canto superior direito de cada página oferece **Reboot System** e **Power Off System**. Ambos agem sobre o par, desligando primeiro o Target e o Host alguns segundos depois, pois o Target só é alcançável através do Host. Cada escolha pede confirmação antes que algo aconteça. Após um desligamento, ambas as máquinas precisam ser ligadas manualmente.

* **Purist Mode App:** Esta página contém os controles para alternar o Modo Purista e seu comportamento de inicialização automática no Target Diretta. Ela se atualiza automaticamente a cada 30 segundos para mostrar o status atual.

* **IR Remote App:** Este link só aparece se você concluiu a configuração do controle remoto IR (Apêndice 2) *e* o Roon Bridge estiver instalado no Host, pois o controle remoto comanda uma Zona do Roon. Se você instalar o Roon Bridge mais tarde, a aba aparecerá sozinha. Esta página fornece um formulário simples para visualizar e atualizar o nome da Zona do Roon que seu controle remoto controlará. Esta página não se atualiza automaticamente, então você pode levar o tempo que precisar para fazer suas edições.

### 🔗 Nota sobre a Funcionalidade Completa da Interface Web

O nível **Super Purist** não aparece na interface web até que você conclua as configurações de hardware e serviços do [**Apêndice 8: Velocidades de Rede Purist Opcionais**](#21-appendix-8-optional-purist-network-speeds). A interface depende dos scripts, sinalizadores e do `limit-speed-100m.service` estabelecidos ali para alterar e manter a velocidade física do enlace, então até lá a página Purist Mode oferece apenas Standard e Purist.

> ---
> ### ✅ Ponto de Controle: Verificar sua Configuração da Interface Web
>
> A Interface Web do Modo Purista deve estar operacional agora. Para verificar todos os componentes deste recurso complexo, prossiga para o [**Apêndice 5**](#18-appendix-5-system-health-checks) e execute o comando de **Verificação de Saúde do Sistema** universal no Host e no Target.
>
> ---

## 18. Apêndice 5: Verificações de Saúde do Sistema
...
## 19. Apêndice 6: Ajuste Opcional de Desempenho em Tempo Real
...
## 20. Apêndice 7: Otimizações Opcionais de IRQ e Threads
...
2.  Inicie a reprodução de música no Target Diretta. Isso garante que o script possa detectar o tráfego de interrupção ativo. Se você não tiver nada à mão, poderá executar este comando no Host:
    ```bash
...
>
> Seu ajuste avançado em tempo real deve estar concluído agora. Para verificar todos os componentes desta nova configuração, por favor, retorne ao [**Apêndice 5**](#18-appendix-5-system-health-checks) e execute o comando de **Verificação de Saúde do Sistema** universal no Host e no Target.
>
> ---

## 21. Apêndice 8: Velocidades Opcionais de Rede Purista
...
>
> Seu link de rede dedicado está agora configurado para a operação "Purista" de 100 Mbps. Para verificar se o serviço do Host está ativo e se o Target negociou corretamente a velocidade (detectado por meio do arquivo marcador), por favor, retorne ao [**Apêndice 5**](#18-appendix-5-system-health-checks) e execute o comando de **Verificação de Saúde do Sistema** universal no Host e no Target.
>
> ---

## 22. Apêndice 9: Otimização de Jumbo Frames Opcional
Esta seção otimiza o transporte para eficiência de alta largura de banda.

#### **Passo 1:** Preparar as Interfaces

Temos de forçar temporariamente as interfaces de rede para a maior MTU jumbo que este kernel aceitar, para verificar o suporte do kernel e preparar o teste de ligação. O bloco tenta **10222** e recua para **9000**; um controlador recusa liminarmente uma MTU acima do seu próprio teto, sem alterar nada, pelo que tentar primeiro a maior não custa nada.
...
case "$end0_mtu" in
  10222) echo "SUCESSO: Este kernel suporta MTU 10222. Avance para o passo 2." ;;
  9000)  echo "SUCESSO: Este kernel suporta MTU 9000. Avance para o passo 2." ;;
  *)     echo "PARE: Este kernel não parece suportar qualquer MTU jumbo." ;;
esac
```

*Se você vir "PARAR" no Host **ou** no Target, não prossiga. O seu kernel não possui o patch necessário.*

*As duas máquinas não têm de coincidir aqui. Este passo indica o que cada **kernel** aceitará: com o kernel que estas imagens trazem, ambas as placas chegam a 10222, ao passo que um Pi 4 com um kernel mais antigo para nos 9000 ou abaixo. Os passos 2 e 3 fixam o par num único patamar testando a própria ligação.*
...
```bash
# 1. Detetar a maior MTU que esta ligação transportará de facto
...
EOF
  ...
  # CycleTime só chega à rede se o Host usar o seu próprio perfil. Se
  # TargetProfileLimitTime não for zero, o perfil de destino automático do
  # Diretta escolhe o ciclo e ignora silenciosamente todos os valores abaixo.
  ...
  if [ "$NEW_MTU" -eq 10222 ]; then
    echo "Otimização: MTU 10222. CycleTime mantido em 1500us."
    ...
    echo "Otimização: MTU 9000. CycleTime relaxado para 1500us."
    sudo sed -i 's/^CycleTime=.*/CycleTime=1500/' /opt/diretta-alsa/setting.inf
    sudo sed -i 's/^InfoCycle=.*/InfoCycle=150000/' /opt/diretta-alsa/setting.inf
  elif [ "$NEW_MTU" -eq 3824 ]; then
    echo "Otimização: MTU 3824. CycleTime relaxado para 1300us."
...
Reinicie primeiro o Target e depois o Host:
```bash
sudo sync && sudo reboot
```

*Com isto a configuração fica concluída. As placas demoram um minuto ou dois a voltar; o que se segue explica de onde vêm estes números e nada lhe pede.*
...
>
> **Porque é que 10222 não relaxa mais o `CycleTime`.** Cada patamar até 9000 gasta os seus bytes adicionais num ciclo mais longo e mais silencioso. Este gasta-os em carga útil e mantém o ciclo nos 1500 µs. O que um ciclo consegue transportar é `(MTU - 2) / CycleTime` bytes por µs: **5.9987** com MTU 9000 e **6.8133** com 10222. O estéreo de 32 bits a 768 kHz precisa de **6.1440**, pelo que a 9000 falha a transmissão única por ciclo por 218 bytes e é enviado como dois quadros; a 10222 cabe com folga. Nada mais nesta plataforma pede mais — DSD512 e 2xDXD precisam ambos de 5.6448 e já cabem a 9000 — por isso este patamar existe para exatamente um formato, e apenas porque dividi-lo por dois quadros é a fragmentação que estes patamares foram criados para evitar.
>
> **10222 é um teto de hardware, não um número redondo.** O controlador `macb` do Raspberry Pi 5 tem um buffer de receção de 10240 bytes; subtraia os 14 bytes de cabeçalho Ethernet e os 4 bytes de FCS e restam 10222. É, por isso, a maior MTU que um Pi 5 sequer aceitará e, consequentemente, o valor mais alto em que qualquer par que inclua um deles pode acordar. Um Pi 4 com um kernel atual anuncia bastante mais — 16347 — mas este guia declina essa folga deliberadamente: alcança uma faixa em que os quadros sobredimensionados se corrompem em silêncio em vez de serem recusados, e não compra nenhum formato que esta plataforma consiga reproduzir. 10222 é o teto que este projeto suporta até que um kernel se revele estável acima disso em **ambas** as placas.
>
...
>
> O modo Super Purista (Apêndice 8) substitui esses valores por 1800 µs em qualquer MTU. Seu link de 10 Mbps limita os formatos suportados a DSD64 e 32 bits, 96 kHz.
***
>
>
> ---
...
> ### ✅ Ponto de Controle: Verificar a Configuração de Rede
>
> Se você conseguiu habilitar o suporte a Jumbo frames para a sua configuração, agora é um bom momento para retornar ao [**Apêndice 5**](#18-appendix-5-system-health-checks) e executar o comando de **Verificação de Saúde do Sistema** universal no Host e no Target.
>
> ---

## 23. Apêndice 10: Atualizações do Sistema Opcionais
...
>    purist-mode --revert
>    ```
> 3. Execute o script de QA universal de **Verificação de Saúde do Sistema** do [**Apêndice 5**](#18-appendix-5-system-health-checks) em **ambos** o Host e o Target.
