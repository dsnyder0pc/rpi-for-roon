...
>
> **なぜ3840ではなく3824なのか：** どちらも同じ限界を異なる層で測っただけの数字です。Raspberry Pi 4の`bcmgenet`ドライバーは受信レディ閾値を16バイト単位で`0xF0`に設定します。つまり**3840バイトの受信バッファ**です。ここから2バイトのアライメント用パディングと14バイトのイーサネットヘッダーを引くと**3824**、すなわち収まる最大のL3 MTUになります。2032の段階も、標準の2048バイトバッファに対する同じ計算です。したがって`mtu 3840`を手動で設定してはいけません。ドライバーはそれを受け付けますが（`mtu 9000`さえ受け付けます）、その後フルサイズのフレームをすべて黙って破棄するため、リンクは上がって見えるのに音楽が流れなくなります。ステップ1はカーネルのサポートを確認しますが、それは必要条件であって十分条件ではありません。実際の上限は、ステップ2と3のpingラダーでしか判明しません。（上流の詳細：[raspberrypi/linux#5561](https://github.com/raspberrypi/linux/issues/5561)）
>
